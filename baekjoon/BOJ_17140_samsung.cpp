...
struct NumCounter {
	...
	void init() {
		...
		index = vi(MAX_SIZE, -1);
		// numCounter에서 count up할 숫자의 위치 저장 -> numCounter[index[number]].second++
		// 방문 여부를 알기 위해 (-1)로 초기화
	}

	// 숫자 반복 횟수 count
	void check(int a) {
		// 인덱스가 초기값(-1)인 경우, 처음 방문한 숫자 
		if (index[a] == -1) {
			index[a] = numCounter.size(); 			// 새로운 index를 설정
			numCounter.push_back(make_pair(a, 1));  // numCounter에 새로 추가
		}
		// 인덱스가 초기값이 아닌 경우, index[a]의 위치에 있는 numCounter pair의 중복 횟수를 up
		else {
			numCounter[index[a]].second++;
		}
	}

	// 1) 중복 횟수가 작은 순서, 2) 숫자가 작은 순서
	static bool comp(pii a, pii b) {
		return (a.second == b.second) ? a.first < b.first : a.second < b.second;
	}

	// numCounter를 참고하여 새로운 line을 만들어 반환
	vi getNewLine() { 
...
struct ArrHandler {
	...
	// row >= col 이면 R연산, 아니면 C연산
	bool isRowBigger() {
		return rowSize >= colSize;
	}

	// 새 row를 구했을 때, 기존 row에 덮어쓰기
	...
	// R연산 : 모든 row에 대해 numCounter로 중복 숫자 체크하고, 새 row와 swap
	void sortRow() {
